import gazpacho from "../assets/images/spain-images/gazpacho.jpg";

const Recipe32 = () => {
  return (
    <div className="flex justify-center bg-custom-cream px-8 py-12 md:py-16">
      <div className="relative w-full max-w-[95%] sm:max-w-4xl lg:max-w-5xl bg-white rounded-lg shadow-lg p-8 md:py-16 top-8">
        {/* Two-column grid for image and ingredients */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-12">
          {/* Left column - Image section */}
          <div className="relative flex justify-center sm:block">
            <div className="absolute left-0 hidden w-24 h-[400px] bg-custom-accent/60 sm:block z-0" />
            <div className="relative w-[80%] sm:w-full sm:ml-12">
              <img
                src={gazpacho}
                alt="Gazpacho"
                className="w-full h-[200px] sm:h-[330px] object-cover rounded-lg shadow-md mt-8 z-10"
              />
              <p className="relative mt-2 text-xs text-center text-gray-500 sm:text-sm 2xl:text-lg sm:text-right italic">
                Image credit -{" "}
                <a
                  href="https://www.freepik.com/free-photo/traditional-spanish-gazpacho-soup-bowl-blue-background_49395258.htm#fromView=image_search_similar&page=1&position=7&uuid=46137d48-d8dc-4f93-9b35-24fd35597b4e&new_detail=true"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-blue-600 hover:text-blue-800"
                >
                  Chandlervid85 on Freepik
                </a>
              </p>
            </div>
          </div>

          {/* Right column - Title and Ingredients */}
          <div className="relative z-10 space-y-8">
            <div className="text-center sm:text-left">
              <h1 className="mb-2 text-2xl text-center font-serif text-custom-accent sm:text-2xl lg:text-4xl 2xl:text-5xl sm:ml-12 lg:ml-24 2xl:ml-20">
                Authentic Gazpacho{" "}
              </h1>
              <p className="text-sm text-center italic text-gray-600 sm:text-md sm:ml-12 lg:ml-24 2xl:text-lg">
                Original recipe from{" "}
                <a
                  href="https://www.gimmesomeoven.com/authentic-gazpacho-recipe/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-blue-600 hover:text-blue-800"
                >
                  gimmesomeoven.com
                </a>
              </p>
            </div>

            <div>
              <h2 className="pl-20 mb-3 text-xl font-serif text-gray-800 sm:text-2xl 2xl:text-4xl 2xl:mb-4">
                Ingredients
              </h2>
              <ul className="pl-20 list-disc space-y-2 text-sm text-gray-700 sm:text-base lg:text-lg 2xl:text-2xl z-10">
                <li>2 pounds ripe Roma tomatoes, halved and cored*</li>
                <li>1 small (1/2 lb) cucumber, peeled and seeded*</li>
                <li>1 medium green bell pepper, cored</li>
                <li>1/2 small red onion, peeled</li>
                <li>2 small garlic cloves (or 1 large clove), peeled</li>
                <li>3 tablespoons olive oil</li>
                <li>2 tablespoons sherry vinegar</li>
                <li>1 teaspoon fine sea salt</li>
                <li>½ teaspoon freshly-cracked black pepper</li>
                <li>½ teaspoon ground cumin</li>
                <li>1 thick slice of white bread, soaked, crusts removed*</li>
                <p>Optional Garnishes</p>
                <li>homemade croutons</li>
                <li>chopped fresh herbs</li>
                <li>a drizzle of olive oil</li>
                <li>any leftover chopped gazpacho ingredients</li>
              </ul>
            </div>
          </div>
        </div>

        <div className="max-w-3xl px-4 mx-auto">
          <h2 className="mb-6 text-xl font-serif text-center text-gray-800 sm:text-2xl 2xl:text-4xl">
            Preparation
          </h2>
          <div className="space-y-4 text-sm text-gray-700 sm:text-base lg:text-lg 2xl:text-2xl">
            <p>
              This refreshing Spanish soup captures the essence of sun-ripened
              summer vegetables. The preparation method, though simple, requires
              attention to detail.
            </p>
            <p>
              The key is using the ripest tomatoes and allowing time for the
              flavors to meld in the refrigerator. While blending time is brief,
              those few minutes transform raw vegetables into a silky,
              well-balanced soup.
            </p>
            <p>
              A tip from my kitchen: For the smoothest texture, remove tomato
              skins by scoring, blanching briefly in boiling water, then
              shocking in ice water - they'll slip right off. And don't skip the
              chilling time - the flavors develop and deepen as the soup rests.
            </p>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Recipe32;
